...
## ☁️ Деплой на Render (онлайн, безкоштовно)

Репозиторій містить **`render.yaml`** (Blueprint): Node-сервер збирається й **сам роздає
зібраний клієнт**, тож усе працює з **одного** сервісу — без окремого хостингу фронтенду й без CORS.

1. Зайдіть на **[render.com](https://render.com)** → **New** → **Blueprint**.
2. Підʼєднайте репозиторій **`uozawo/Battleship`** (гілка `main`).
3. Render прочитає `render.yaml`, створить web-сервіс і сам згенерує `JWT_SECRET`.
4. За кілька хвилин гра буде онлайн за адресою `https://<назва>.onrender.com`.
...
| Node | 24 (`NODE_VERSION` / `.node-version`) |

> ⚠️ **Безкоштовний тариф:** сервіс «засинає» після ~15 хв без запитів і прокидається
> ~30–60 с при першому заході. БД — `node:sqlite` на ефемерному диску, тож акаунти й
> лідерборд **скидаються** при передеплої та пробудженні. Для постійного збереження
> підключіть платний Render Disk (і задайте `DB_PATH` на нього) або зовнішню БД.
